...
module.exports.run = async (Client, interaction, slasherrorlog) => {
  ...
  try {
    ...
    if (user.roles.cache.get(mutedRole.id)) return interaction.reply({ content: '**This member is already Muted!**', ephemeral: true })
    ...
    if (mutedRole) {
      ...
      // it will send this interaction once the bot mute the member
      let embed7 = new discord.MessageEmbed()

        .setColor('GREEN')
        .setDescription(`**${user.user.tag} is now muted.**`)
      ...
      const embed = new discord.MessageEmbed()
        .setColor('RED')
        .setTitle('Action Took: User got muted')
        ...
        embeds:
          [
            new discord.MessageEmbed()
              ...
              .setDescription(`You are muted in the server **This is AoT** by the moderation team\n`)
              .addFields(
                { name: 'Reason:', value: `\`${reason}\`` },
              )
      ...
      interaction.reply({ content: '**Unable to find any Role to mute the user.**', ephemeral: true })
    }

  } catch (err) {
    console.log(err)
    slasherrorlog(err, interaction, 'mute')
  }
