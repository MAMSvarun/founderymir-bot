...
module.exports.run = async (Client, message, args, prefix, errorlog) => {
    ...
    if (!oppenent) return message.reply({content: `**Mention a user to play Connect4 with them.**`}).then(msg => {
        setTimeout(() => {
            msg.delete()
            message.delete()
        }, 5000);
    ...
    try {
        ...
        const initial = new discord.MessageEmbed()
            .setTitle(`'🔴' It's your turn, ${message.author.tag}!`)
            .setDescription(initialState)
            .setFooter(`${challenger.tag} vs ${oppenent.tag}`)
                ...
                if (user.id === gameData[player].member.id) {
                    ...
                    switch (reaction.emoji.name) {
                        case "1️⃣":
                            ...
                            if (openSpaces.length == 0) return message.channel.send(`**${gameData[player].member}, that column is already full. Choose another one**`).then(msg1 => msg1.delete({ timeout: 10000 }))
                    ...
                    for (const func of checks) {
                        ...
                        if (data) {
                            ...
                            const WinEmbed = new discord.MessageEmbed()
                                .setTitle(`${gameData[player].member.tag} has won the game!`)
                                .setDescription(renderBoard(board))
                                .setFooter(`${challenger.tag} vs ${oppenent.tag}`)
                    ...
                    const newEmbed = new discord.MessageEmbed()
                        .setTitle(`${gameData[player].playerColor} -  It's your turn, ${gameData[player].member.tag}!`)
                        .setDescription(renderBoard(board))
                        .setFooter(`${challenger.tag} vs ${oppenent.tag}`)
